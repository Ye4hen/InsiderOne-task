...
const isFinished = computed(() => raceStore.currentRoundIndex >= ROUNDS_COUNT)
const currentRoundNumber = computed(() => raceStore.currentRoundIndex + 1)
const hasStarted = computed(() => raceStore.currentRoundIndex >= 0)

const currentRoundText = computed(() => {
  if (isFinished.value) return 'Race is finished'
  else if (hasStarted.value && currentRoundNumber.value !== 0) return currentRoundNumber.value
  else return 'Race has not started, yet'
})
...
<template>
  <div class="race-track">
    ...
    <div v-else class="track-container section-container">
